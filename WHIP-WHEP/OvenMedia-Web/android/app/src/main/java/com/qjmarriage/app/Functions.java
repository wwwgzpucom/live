...
public class Functions{
	...
	public boolean url_actions(WebView view, String url, Activity activity) {
		...
		if (url.startsWith("refresh:")) {
			...
			try {
				...
				Toast.makeText(context, R.string.no_dialer_app, Toast.LENGTH_SHORT).show();
			}
			return true;
			...
			if(permissionManager.isNotificationPermissionGranted()) {
				...
				Toast.makeText(context, R.string.notification_permission_required, Toast.LENGTH_SHORT).show();
			}
			return true;
		...
		if (url.startsWith("http:") || url.startsWith("https:")) {
			...
			if (SWVContext.ASWP_EXTURL && !aswm_host(url).equals(SWVContext.ASWV_HOST) && !isExceptedExternalHost(aswm_host(url))) {
	...
	/**
	 * Match host against comma-separated {@link SWVContext#ASWV_EXC_LIST}
	 * (OME / live CDN hosts should stay inside WebView).
	 */
	public static boolean isExceptedExternalHost(String host) {
		if (host == null || host.isEmpty()) {
			return false;
		}
		String list = SWVContext.ASWV_EXC_LIST;
		if (list == null || list.trim().isEmpty()) {
			return false;
		}
		String needle = host.trim().toLowerCase();
		for (String part : list.split(",")) {
			String item = part.trim().toLowerCase();
			if (!item.isEmpty() && (needle.equals(item) || needle.endsWith("." + item))) {
				return true;
			}
		}
		return false;
