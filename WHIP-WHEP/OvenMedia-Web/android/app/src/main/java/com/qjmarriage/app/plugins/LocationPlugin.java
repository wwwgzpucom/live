...
import com.qjmarriage.app.PermissionManager;
import com.qjmarriage.app.PluginInterface;
import com.qjmarriage.app.PluginManager;
import com.qjmarriage.app.PrivacyConsent;
...
public class LocationPlugin implements PluginInterface, LocationListener {
    ...
    public void initialize(Activity activity, WebView webView, Functions functions, Map<String, Object> config) {
        ...
        // Do not touch LocationManager / location APIs here — 应用宝 requires privacy consent first.
        webView.addJavascriptInterface(new LocationJSInterface(), "LocationInterface");
        Log.d(TAG, "LocationPlugin initialized (location fetch deferred until consent + use).");
    }
    ...
    public void getLocation(String jsCallbackName) {
        ...
        if (!PrivacyConsent.isAccepted(activity)) {
            Log.w(TAG, "Blocked getLocation: privacy not accepted.");
            sendLocationError("Privacy policy not accepted.");
            return;
        }
        ...
        try {
            ...
            if (locationManager == null) {
                sendLocationError("Location service unavailable.");
                return;
            }
